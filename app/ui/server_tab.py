...
class ServerTabMixin:
    ...
    def _build_server_tab(self) -> None:
        ...
        self.server_dir_edit = QLineEdit(self.config.server_dir)
    ...
    def _select_server_directory(self) -> None:
        folder = QFileDialog.getExistingDirectory(self, "Выберите папку VoiceEmotionServer", self.server_dir_edit.text().strip() or self.config.server_dir)
        if folder:
    ...
    def _save_server_settings(self, show_message: bool = True) -> None:
        if hasattr(self, "server_url_edit"):
            self.config.server_url = self.server_url_edit.text().strip().rstrip("/") or "http://127.0.0.1:8000"
        if hasattr(self, "server_dir_edit"):
            self.config.server_dir = self.server_dir_edit.text().strip() or r"C:\VoiceEmotionServer"
    ...
    def _verify_server_environment(self) -> None:
        self._save_server_settings(show_message=False)
        self._run_server_action("Проверка серверной папки", verify_server_environment, self.config.server_dir)

    def _refresh_server_status(self) -> None:
        self._save_server_settings(show_message=False)
        self._run_server_action("Состояние сервера", server_status, self.config.server_dir, self.config.server_url)

    def _show_server_logs(self) -> None:
        self._save_server_settings(show_message=False)
        service = self.server_log_service_combo.currentText()
        self._run_server_action(f"Логи {service}", service_logs, self.config.server_dir, service, 300)

    def _open_server_directory(self) -> None:
        try:
            self._save_server_settings(show_message=False)
            open_directory(validate_server_directory(self.config.server_dir))
    ...
    def _open_backups_directory(self) -> None:
        try:
            self._save_server_settings(show_message=False)
            open_directory(validate_server_directory(self.config.server_dir) / "backups")
        except Exception as exc:
            self._show_error("Резервные копии", str(exc))

    def _open_server_docs(self) -> None:
        self._save_server_settings(show_message=False)
        open_url(self.config.server_url.rstrip("/") + "/docs")

    def _create_registration_code(self) -> None:
        self._save_server_settings(show_message=False)
        minutes = self.registration_minutes_spin.value()
        worker = FunctionWorker(create_registration_code, self.config.server_url, self.config.server_dir, minutes)
        def done(result) -> None:
    ...
    def _refresh_server_models(self) -> None:
        if not hasattr(self, "server_models_table"):
            return
        self._save_server_settings(show_message=False)
        worker = FunctionWorker(list_server_models, self.config.server_url, self.config.server_dir)
    ...
    def _activate_selected_server_model(self) -> None:
        ...
        if answer != QMessageBox.Yes:
            return
        worker = FunctionWorker(activate_server_model, self.config.server_url, self.config.server_dir, version)
        def done(result) -> None:
    ...
    def _refresh_server_devices(self) -> None:
        self._save_server_settings(show_message=False)
        worker = FunctionWorker(list_server_devices, self.config.server_url, self.config.server_dir)
