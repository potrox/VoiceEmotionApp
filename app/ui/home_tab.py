"""Главная вкладка со сводной информацией о проекте."""
...
class HomeTabMixin:
    ...
    def _refresh_home(self) -> None:
        ...
        try:
            ...
            last_experiments = experiment_database.last_experiments(1)
            if last_experiments:
                row = last_experiments[0]
                lines.append(f"Последнее обучение: {row.get('created_at')}")
                lines.append(f"Последняя сохранённая модель: {row.get('model_path', '')}")
                best_model_name = str(row.get("best_model_name") or "").strip()
                if best_model_name:
                    metrics = json.loads(row.get("metrics_json") or "{}")
                    best_evaluation = metrics.get(best_model_name, {})
                    lines.append(f"Последняя лучшая модель: {best_model_name}")
                    if best_evaluation:
                        lines.append(
                            f"Macro F1: {best_evaluation.get('macro_f1', 0):.3f}; "
                            f"balanced accuracy: {best_evaluation.get('balanced_accuracy', 0):.3f}"
                        )
        except Exception:
            pass
        unique_lines = []
