...
from .gui_dialogs import EmotionMappingDialog, RecoveryMetadataDialog
...
class DatasetTabMixin:
    ...
    def _recover_user_dataset_csv_on_startup(self) -> None:
        try:
            base = self.paths.datasets / "user_dataset"
            csv_path = base / "user_dataset.csv"
            records = scan_user_dataset_recordings(base, sample_rate=self.cfg.sample_rate)
    ...
    def _manual_rebuild_user_dataset_csv(self) -> None:
        try:
            ...
            if not records:
                ...
                return
            health = check_user_dataset_csv(csv_path, records)
            reason = health.reason if health.needs_rebuild else "Принудительное восстановление по запросу пользователя."
    ...
    def _build_dataset_tab(self) -> None:
        ...
        self.validate_audio_checkbox.setChecked(self.cfg.validate_external_audio)
    ...
    def _select_dataset_csv(self) -> None:
        path, _ = QFileDialog.getOpenFileName(self, "Выберите CSV-датасет", str(self.paths.datasets), "CSV (*.csv)")
        if not path:
            return
        ...
        try:
            ...
            try:
                ...
                if selected_path.name.lower() == "user_dataset.csv" and "user_dataset" in parts:
                    records = scan_user_dataset_recordings(selected_path.parent, sample_rate=self.cfg.sample_rate)
                    if records:
                        answer = QMessageBox.question(
                            self,
                            "CSV повреждён",
        ...
        worker = DatasetLoadWorker(
            path,
            self.cfg.min_train_duration_sec,
