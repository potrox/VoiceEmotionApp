...
from .constants import AGE_GROUPS, EMOTION_RU, EMOTIONS, GENDERS, LANGUAGES, STANDARD_PHRASES
...
class RecordingTabMixin:
    def _build_record_tab(self) -> None:
        ...
        self.speaker_edit.setText(UserDatasetRecorder(self.paths.datasets, self.cfg.sample_rate).next_speaker_id())
        self.gender_combo = QComboBox(); self.gender_combo.addItems(GENDERS)
        self.age_combo = QComboBox(); self.age_combo.addItems(AGE_GROUPS)
        self.lang_combo = QComboBox(); self.lang_combo.addItems(LANGUAGES)
        self.record_emotion_combo = QComboBox(); self.record_emotion_combo.addItems([f"{e} — {EMOTION_RU[e]}" for e in EMOTIONS])
        self.phrase_combo = QComboBox(); self.phrase_combo.addItems(STANDARD_PHRASES); self.phrase_combo.setEditable(True)
        self.record_duration_spin = QSpinBox(); self.record_duration_spin.setRange(3, 10); self.record_duration_spin.setValue(self.cfg.record_duration_sec)
        ...
        self.record_log = QTextEdit(); self.record_log.setReadOnly(True)
    ...
    def _fill_device_combo(self, combo: QComboBox | None) -> None:
        ...
        for dev in devices:
            suffix = " — по умолчанию" if dev.get("is_default") else ""
            sr = dev.get("default_samplerate") or "?"
            text = f"#{dev['index']} {dev['name']} ({dev['channels']} канал., {sr} Гц){suffix}"
            combo.addItem(text, int(dev["index"]))
        preferred = self.cfg.input_device_index
        for i in range(combo.count()):
            if combo.itemData(i) == current or combo.itemData(i) == preferred:
    ...
    def _close_live_record_stream(self) -> None:
        """Close PortAudio stream without waiting for the driver to finish naturally."""
        stream = getattr(self, "live_record_stream", None)
    ...
    def _record_timeout_guard(self, generation: int) -> None:
        """Safety guard: finish the recording from the Qt timer if something went wrong."""
    ...
    def _record_voice(self) -> None:
        if self.is_recognition_mic_recording:
            self._show_info("Запись уже идёт", "Сначала завершите запись во вкладке «Распознавание».")
            return
        if self.is_recording_now:
            self._show_info("Запись уже идёт", "Дождитесь завершения текущей записи или нажмите «Отменить запись».")
            return
        ...
        self.live_record_blocks = []
        self.live_record_statuses = []
        self.live_record_received_frames = 0
        duration = int(self.record_duration_spin.value())
        device_label = self.record_device_combo.currentText()
        ...
        self.live_record_actual_sr = actual_sr
        self.live_record_expected_frames = max(1, int(duration * actual_sr))

        def audio_callback(indata, frames, _time_info, status):
            if status:
                text = str(status)
                if text and text not in self.live_record_statuses:
                    self.live_record_statuses.append(text)
            if not self.is_recording_now:
                return
            remaining = self.live_record_expected_frames - self.live_record_received_frames
            if remaining <= 0:
                return
            take = min(int(frames), int(remaining))
            if take > 0:
                self.live_record_blocks.append(np.array(indata[:take, 0], dtype=np.float32, copy=True))
                self.live_record_received_frames += take

        try:
            self.live_record_stream = sd.InputStream(
        ...
        self._set_record_cache_status(
            "Кэш записи: идёт запись. После завершения временный WAV будет сразу сохранён в cache/."
        )
        self._start_record_timer_progress(duration)
        self.record_log.append(
            f"Начата запись на {duration} сек. Говорите сейчас. Устройство: {device_label}; частота: {actual_sr} Гц."
        )

        QTimer.singleShot(duration * 1000, lambda: self._finish_live_recording(generation, forced=False))
        QTimer.singleShot((duration + 3) * 1000, lambda: self._record_timeout_guard(generation))

    def _finish_live_recording(self, generation: int, forced: bool = False) -> None:
        if generation != self.recording_generation or not self.is_recording_now:
            return
        self._set_recording_ui_state(False)
        self._stop_record_timer_progress(100 if not forced else 0)
        self._close_live_record_stream()

        blocks = list(self.live_record_blocks)
        self.live_record_blocks = []
        if not blocks:
            self.recording_buffer = None
            self._clear_recording_cache_file()
            self._set_record_cache_status("Кэш записи: пусто — микрофон не передал аудиоданные.")
            self.record_log.append("Запись завершена без аудиоданных. Проверьте выбранное устройство и разрешения Windows.")
            return

        y = np.concatenate(blocks).astype(np.float32)
        if y.size < self.live_record_expected_frames:
            y = np.pad(y, (0, self.live_record_expected_frames - y.size), mode="constant")
        elif y.size > self.live_record_expected_frames:
            y = y[: self.live_record_expected_frames]
        try:
            y = _resample_if_needed(y, self.live_record_actual_sr, self.cfg.sample_rate)
            cache_path = self.paths.cache / f"microphone_cache_{timestamp()}.wav"
            save_wav(cache_path, y, self.cfg.sample_rate)
            quality = analyze_quality(y, self.cfg.sample_rate, training=False)
        except Exception as exc:
            self.recording_buffer = None
            self._clear_recording_cache_file()
            self._set_record_cache_status("Кэш записи: пусто — не удалось сохранить временный WAV.")
            self._show_error("Ошибка сохранения записи", str(exc))
            return

        self.recording_buffer = y
        self.recording_cache_path = Path(cache_path)
        self._set_record_cache_status(
            f"Кэш записи: есть временная запись — {self.recording_cache_path}. "
            "Нажмите «Сохранить в датасет», чтобы перенести её в datasets/, или «Удалить текущую запись»."
        )
        self.record_log.append(
            f"Запись завершена. Получено кадров: {len(y)}; длительность: {quality.duration_sec:.2f} сек.; RMS: {quality.rms:.5f}."
        )
        self.record_log.append(f"Временный файл в кэше: {self.recording_cache_path}")
        if self.live_record_statuses:
            self.record_log.append("Предупреждения аудиодрайвера: " + "; ".join(self.live_record_statuses))
        if quality.warnings:
            for w in quality.warnings:
                self.record_log.append("Предупреждение: " + w)
        else:
    ...
    def _play_recording(self) -> None:
        ...
        if self.recording_cache_path and Path(self.recording_cache_path).exists():
            ...
            try:
                play_path = self.paths.cache / "microphone_cache_preview.wav"
                save_wav(play_path, self.recording_buffer, self.cfg.sample_rate)
    ...
    def _save_recording(self) -> None:
        ...
        recorder = UserDatasetRecorder(self.paths.datasets, self.cfg.sample_rate)
        try:
    ...
    def _import_phrases(self) -> None:
        if self.is_recording_now:
            ...
            return
        path, _ = QFileDialog.getOpenFileName(self, "Выберите TXT с фразами", str(self.paths.datasets), "Text (*.txt)")
        if not path:
            return
        try:
        ...
        existing = {self.phrase_combo.itemText(i) for i in range(self.phrase_combo.count())}
        added = 0
        for line in lines:
    ...
    def _refresh_record_progress(self) -> None:
        csv_path = self.paths.datasets / "user_dataset" / "user_dataset.csv"
