...
class EmotionEventInput(BaseModel):
    ...
    def validate_probabilities(self) -> "EmotionEventInput":
        supported_classes = (set(EMOTIONS), {"joy", "sadness", "anger", "calm"})
        if set(self.probabilities) not in supported_classes:
            raise ValueError("Вероятности должны содержать четыре или семь поддерживаемых эмоций")
